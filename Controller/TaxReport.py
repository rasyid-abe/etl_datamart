...
from Models import TaxReportMart
...
class sequencePerMin(object):
    ...
    def on_get(self, req, resp):
        ...
        content= TaxReportMart.processSequencePerMin(seq, sIDStore = sIDStore , n_size_insert= insert, n_size_delete= delete,
                                                                core_spec=core,\
...
class sequence5min(object):
    ...
    def on_get(self, req, resp):
        ...
        insert = req.get_param_as_int('size_insert') if (req.get_param_as_int('size_insert')) else 1000
        delete = req.get_param_as_int('size_delete') if (req.get_param_as_int('size_delete')) else 1000
        content= TaxReportMart.processSequencePerMin(5, n_size_insert = insert, n_size_delete = delete)
        data = {'status':'success','content':content,'enum':'1'}
        resp.body = json.dumps(data)
...
class sequence45min(object):
    ...
    def on_get(self, req, resp):
        ...
        insert = req.get_param_as_int('size_insert') if (req.get_param_as_int('size_insert')) else 1000
        delete = req.get_param_as_int('size_delete') if (req.get_param_as_int('size_delete')) else 1000
        content= TaxReportMart.processSequencePerMin(45, n_size_insert = insert, n_size_delete = delete)
        data = {'status':'success','content':content,'enum':'1'}
        resp.body = json.dumps(data)
...
class sequence120min(object):
    ...
    def on_get(self, req, resp):
        ...
        insert = req.get_param_as_int('size_insert') if (req.get_param_as_int('size_insert')) else 1000
        delete = req.get_param_as_int('size_delete') if (req.get_param_as_int('size_delete')) else 1000
        content= TaxReportMart.processSequencePerMin(120, n_size_insert = insert, n_size_delete = delete)
        data = {'status':'success','content':content,'enum':'1'}
        resp.body = json.dumps(data)
...
class withParam(object):
    def on_get(self, req, resp):
        ...
        content = TaxReportMart.processParam("pyspark-akunting-jurnal-param", sEmail, stime, etime, idCabang = idOutlet)
        data = {'status': 'success', 'content': content, 'enum': '1'}
        resp.body = json.dumps(data)
...
class Kafka(object):
    ...
    def on_get(self, req, resp):
        ...
        content = TaxReportMart.processTaxReportSequenceKafka(topic_name, max_message, timeout)
        data = {'status': 'success', 'content': content, 'enum': '1'}
        resp.body = json.dumps(data)
