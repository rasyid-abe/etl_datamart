...
class Daily(ETL):
    ...
    def _extract(self, filters:list, postfix:str):
        ...
        self.pushCachedTable(df, f'df_salesdashboard_mart_{postfix}')
        self.timelog['extract_mart_daily'] = time.time() - _timestamp
        return f'df_salesdashboard_mart_{postfix}'
    ...
    def _transform(self, name:str) -> DataFrame :
        ...
        spark_query = f"""
                SELECT
                    ...
                    SUM(IF (raw_mart_sales_dashboard_is_refund , 0 , raw_mart_sales_dashboard_sales_value )) mart_sales_dashboard_daily_sales_value,
                    SUM(IF (raw_mart_sales_dashboard_is_refund , 0 , raw_mart_sales_dashboard_payment_value )) mart_sales_dashboard_daily_payment_value,
                    SUM (
                    ...
                    SUM(IF (raw_mart_sales_dashboard_is_refund , 0 , raw_mart_sales_dashboard_gross_sales )) mart_sales_dashboard_daily_gross_sales,
                    SUM(IF (raw_mart_sales_dashboard_is_refund , 0 , raw_mart_sales_dashboard_product )) mart_sales_dashboard_daily_product_qty,
                    SUM(raw_mart_sales_dashboard_count_transaction) mart_sales_dashboard_daily_transaction,
                    SUM(IF (raw_mart_sales_dashboard_is_refund , raw_mart_sales_dashboard_refund , 0 )) mart_sales_dashboard_daily_refund,
                    (
    ...
    def delete(self, filters:list):
        _timestamp = time.time()
        sql = f"DELETE FROM mart_prd.mart_sales_dashboard_daily WHERE (m_user_id_user, mart_sales_dashboard_daily_date, mart_sales_dashboard_daily_order_type, m_cabang_id_cabang) IN ({','.join([repr(filter) for filter in filters])})"
