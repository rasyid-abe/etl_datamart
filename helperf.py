...
def get_spark_config(appName , master = "spark://spark-master:7077", driver_memory:float=10000, executor_memory:float=50000, executor_core:int=7):
    getSparkSpecConfig() # add spec from .env and environment variables to config
    ...
    conf.set("spark.driver.memory", "{}m".format(config.get('DRIVER_MEMORY') or driver_memory))
    conf.set("spark.executor.memory", "{}m".format(config.get('EXECUTOR_MEMORY') or executor_memory))
    conf.set("spark.executor.cores", "{}".format(config.get('EXECUTOR_CORE') or executor_core))
...
def getENV():
    return str(config['ENV']) if 'ENV' in config else "PRODUCTION"

def getSparkSpecConfig():
    config['DRIVER_MEMORY'] = os.environ.get('DRIVER_MEMORY', config.get('DRIVER_MEMORY'))
    config['EXECUTOR_MEMORY'] = os.environ.get('EXECUTOR_MEMORY', config.get('EXECUTOR_MEMORY'))
    config['EXECUTOR_CORE'] = os.environ.get('EXECUTOR_CORE', config.get('EXECUTOR_CORE'))
